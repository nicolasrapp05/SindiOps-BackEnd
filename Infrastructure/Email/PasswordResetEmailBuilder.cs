...
public static class PasswordResetEmailBuilder
{
    public static (string HtmlContent, string PlainBody) Build(
        string email, string otp, string frontendUrl, string hashedToken)
    {
        var encodedEmail = WebUtility.HtmlEncode(email);
        var encodedOtp = WebUtility.HtmlEncode(otp);
        var resetUrl = RecoveryUrlBuilder.Build(frontendUrl, "redefinir-senha", hashedToken);
        ...
        var htmlContent = $"""
            ...
            <p style="margin:0 0 16px;font-size:13px;color:#71717a;">
              <strong>Importante:</strong> use apenas este email. Não utilize links ou códigos de mensagens anteriores.
        ...
        var plainBody = $"""
            ...
            Ou use o código de verificação: {otp}

            Importante: use apenas este email. Não utilize links ou códigos de mensagens anteriores.
            O código expira em algumas horas e só pode ser usado uma vez.
